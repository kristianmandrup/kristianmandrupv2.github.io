...
To modify your profile, simply edit `data.yaml` in the project root.
Then convert to a `resume.json` file in `public` folder as follows:

`npx yaml2json data.yaml --pretty > public/resume.json`

To serve the json file, make sure you configure the path correctly. You might have different paths in `dev` mode vs `production` mode. You can control this in `getResumePath`:
